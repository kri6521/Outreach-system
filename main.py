...
# 1️⃣ Search tool (SerpAPI under the hood) – from langchain-community
...
def draft_email(name: str, site: str) -> str:
    ...
    """
    prompt = f"""
You are a sales outreach assistant helping me craft a personalized cold email.

TARGET: Decision-maker at {name} ({site}) in the textile industry.

TONE: Professional but approachable — like a peer reaching out with a helpful solution.

TASK: Draft a concise, personalized cold email that:
1. Opens with a relevant compliment or observation about {name}'s business
2. Briefly introduces our AI-powered fabric sourcing platform
3. Highlights ONE key benefit (e.g., faster supplier matching, cost reduction, quality assurance)
4. Includes a clear call-to-action: 10-minute call or demo

Keep it under 120 words. Avoid jargon. Make it feel human and not templated.
"""
